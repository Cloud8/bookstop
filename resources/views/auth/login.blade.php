...
<div class="min-h-[calc(100vh-8rem)] flex items-center justify-center px-4 py-12">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            <h1 class="font-serif text-3xl text-text-primary mb-2">Вход в аккаунт</h1>
            <p class="text-sm text-text-muted">Рады видеть вас снова</p>
        </div>
                ...
                <div class="flex items-center justify-between">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input
                            ...
                            type="checkbox"
                            name="remember"
                            class="rounded border-border-subtle text-brand-600 focus:ring-brand-500"
                        >
                        <span class="text-sm font-sans text-text-muted">Запомнить меня</span>
                    </label>

                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-sm font-sans text-brand-600 hover:text-brand-800 transition">
                            Забыли пароль?
                        </a>
                    @endif
                </div>
        ...
        <p class="mt-6 text-center text-sm text-text-muted font-sans">
            Нет аккаунта?
            <a href="{{ route('register') }}" class="text-brand-600 hover:text-brand-800 transition font-medium">
                Зарегистрироваться
            </a>
        </p>
